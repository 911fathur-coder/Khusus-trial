...
class App{
  ...
  cacheDom(){
    ...
    this.dom = {
      ...
      devBlockedListBtn:$('devBlockedListBtn'),
      deviceBlockedScreen:$('deviceBlockedScreen'), deviceBlockedReason:$('deviceBlockedReason'),
      ...
      timingStopList:$('timingStopList'), timingAddStopBtn:$('timingAddStopBtn'),
  ...
  /* =========================================================
     DEVICE BLOCK CHECK — dijalankan sedini mungkin saat app dibuka.
     Kalau device ini ada di koleksi Firestore "blocked_devices",
     layar full-screen "Akses Diblokir" langsung tampil dan menutupi
     seluruh app (termasuk consent gate), tanpa tombol tutup.
     Kalau device offline / Firestore tidak bisa diakses saat cek ini,
     app TETAP BOLEH DIPAKAI (fail-open) — konsisten dengan app yang
     memang didesain bisa dipakai offline dan sinkron belakangan.
     ========================================================= */
  async initDeviceBlockCheck(){
    if(!window.firebaseDb) return;
    try{
      const deviceId = this.getDeviceId();
      const doc = await window.firebaseDb.collection('blocked_devices').doc(deviceId).get();
      if(doc.exists) this.showDeviceBlockedScreen(doc.data());
    }catch(e){ /* offline / gagal cek — biarkan tetap bisa dipakai */ }
  }
  showDeviceBlockedScreen(data){
    if(data && data.reason){
      this.dom.deviceBlockedReason.textContent = 'Catatan: '+data.reason;
      this.dom.deviceBlockedReason.classList.remove('hidden');
    }
    this.dom.deviceBlockedScreen.classList.add('show');
  }
  ...
  APPS = [
    ...
    { id:'timing', title:'Output Finish Good OTB', desc:'Jadwal output per shift', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3.5 2"/></svg>', ready:true },
  ...
  switchApp(appId){
    ...
    if(appId==='doubleseam') this.applyDoubleSeamHeader();
      ...
      this.dom.largeTitle.textContent = 'Output Finish Good OTB';
      this.dom.navCompactTitle.textContent = 'Finish Good OTB';
      this.dom.largeTitleSub.textContent = 'JADWAL OTB · PRESENTED BY FATHUR';
  ...
  initTimingOutput(){
    ...
    this.dom.timingAddStopBtn.addEventListener('click', ()=>this.timingAddStopRow());
    this.timingStopIdCounter = 0;
    this.dom.timingStopList.addEventListener('click', (e)=>{
      const btn = e.target.closest('.timing-stop-remove');
      if(btn) btn.closest('.timing-stop-row').remove();
    });
  }

  /* Time Stop: blok waktu tambahan yang opsional & jumlahnya bebas —
     bekerja seperti Jam Istirahat (dikecualikan dari jadwal cek),
     tapi user bisa menambah beberapa sekaligus (mis. ada 2-3 stop
     mesin terjadwal dalam 1 shift). Baris kosong (belum diisi)
     otomatis diabaikan saat Generate. */
  timingAddStopRow(){
    this.timingStopIdCounter++;
    const id = this.timingStopIdCounter;
    const row = document.createElement('div');
    row.className = 'timing-stop-row';
    row.dataset.stopId = id;
    row.innerHTML = '<div class="timing-stop-field"><label>Dari</label><input type="text" class="timing-stop-start" placeholder="HH:MM"></div>'
      + '<div class="timing-stop-field"><label>Sampai</label><input type="text" class="timing-stop-end" placeholder="HH:MM"></div>'
      + '<button type="button" class="timing-stop-remove" aria-label="Hapus Time Stop"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg></button>';
    this.dom.timingStopList.appendChild(row);
    vibrate(8);
  }
  ...
  /* Menormalkan satu window Dari/Sampai ke skala menit relatif terhadap
     start shift, dengan penanganan lewat-tengah-malam yang sama persis
     seperti window Jam Istirahat aslinya. */
  timingNormalizeWindow(s, e, start, end){
    const sRel = (s < start && s + 1440 < end) ? s + 1440 : s;
    const eRel = (e <= sRel) ? e + 1440 : e;
    return [sRel, eRel];
  }
  ...
  timingCalculate(){
    ...
    // Kumpulkan semua blok waktu yang dikecualikan: Jam Istirahat (selalu ada)
    // + Time Stop opsional (baris yang Dari/Sampai-nya kosong diabaikan).
    let blockedWindows = [ this.timingNormalizeWindow(bS, bE, start, end) ];
    this.dom.timingStopList.querySelectorAll('.timing-stop-row').forEach(row=>{
      const sVal = row.querySelector('.timing-stop-start').value.trim();
      const eVal = row.querySelector('.timing-stop-end').value.trim();
      if(!sVal || !eVal) return;
      const s = this.timingParseTime(sVal), e = this.timingParseTime(eVal);
      blockedWindows.push(this.timingNormalizeWindow(s, e, start, end));
    });
    // Urutkan & gabungkan window yang saling tumpang tindih (mis. Time Stop
    // yang kepotong Jam Istirahat) supaya tidak dihitung dobel.
    blockedWindows.sort((a,b)=>a[0]-b[0]);
    const mergedWindows = [];
    blockedWindows.forEach(w=>{
      const last = mergedWindows[mergedWindows.length-1];
      if(last && w[0] <= last[1]) last[1] = Math.max(last[1], w[1]);
      else mergedWindows.push([w[0], w[1]]);
    });

    const targetEnd = end - Math.floor(Math.random() * 8);
    let blockedOverlap = 0;
    mergedWindows.forEach(([s,e])=>{
      if(s < targetEnd && e > start) blockedOverlap += (Math.min(targetEnd, e) - Math.max(start, s));
    });

    let netDur = (targetEnd - start) - blockedOverlap;
    let numGaps = count - 1;
    ...
    // BUILD SCHEDULE — lompati setiap blok (Jam Istirahat / Time Stop) yang
    // terlewati dalam satu langkah gap, sama seperti logika aslinya, cuma
    // sekarang diterapkan untuk semua window bukan cuma satu.
    let schedule = [start];
    let curr = start;
    for (let g of gaps) {
      const oldCurr = curr;
  ...
  spcSlitterSwitchParam(paramKey){
    ...
    } else if(paramKey==='length'){
      // Body Blank Length cuma punya 6 titik (beda dari Height yang 8/9) — toggle Lajur 9 tidak relevan.
      this.dom.spcSlitterLajur9Wrapper.classList.add('hidden');
  ...
  /* Body Blank Height pakai toggle Lajur 9 (8 atau 9 kolom). Body Blank
     Length SELALU 6 kolom saja — tidak ada varian 8/9 untuk parameter ini. */
  spcSlitterGetTotalCols(){
    if(this.spcSlitterParam==='height') return this.dom.spcSlitterLajur9Toggle.checked ? 9 : 8;
    if(this.spcSlitterParam==='length') return 6;
    return 8;
  }
  ...
  spcSlitterHandleAutoTab(e){
    ...
    if(val.length===targetLength){
      ...
      if(this.spcSlitterParam==='height' || this.spcSlitterParam==='length'){
        const maxCol = this.spcSlitterGetTotalCols();
        if(row==='A') nextRow='B';
        else if(row==='B') nextRow='C';
  ...
  spcSlitterRenderInputTable(){
    ...
    let html = '<table class="spc-table"><thead><tr><th>Titik</th>';
    if(this.spcSlitterParam==='height' || this.spcSlitterParam==='length'){
      const totalCols = this.spcSlitterGetTotalCols();
  ...
  spcSlitterGenerate(){
    ...
    if(this.spcSlitterParam==='height' || this.spcSlitterParam==='length'){
      const useLajur9 = this.spcSlitterParam==='height' ? this.dom.spcSlitterLajur9Toggle.checked : false;
      const totalCols = this.spcSlitterGetTotalCols();
  ...
  buildSpcSlitterPayload(header, checks){
    ...
    if(param==='height' || param==='length'){
      const pockets = {};
      const totalCols = param==='length' ? 6 : (useLajur9 ? 9 : 8);
  ...
  initDevAccess(){
    ...
    this.dom.devBlockedListBtn.addEventListener('click', ()=>this.openBlockedDevicesManager());
  ...
  renderDevLogList(){
    ...
    this.dom.devLogListArea.innerHTML = '<div class="dev-log-count">Menampilkan '+filtered.length+' dari '+all.length+' log'+(all.length>=this.DEV_LOG_LIMIT?' (mungkin ada lebih banyak)':'')+'</div>'
      ...
      return '<div class="dev-log-card" data-doc-id="'+d._id+'">'
        ...
        + (d.deviceId ? '<button class="dev-log-block" data-device-id="'+d.deviceId+'" data-device-label="'+(d.deviceLabel||'')+'" aria-label="Blokir device ini"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M5.5 5.5l13 13"/></svg></button>' : '')
    ...
    this.dom.devLogListArea.querySelectorAll('.dev-log-block').forEach(btn=>{
      btn.addEventListener('click', (e)=>{
        e.stopPropagation();
        const deviceId = btn.dataset.deviceId;
        const deviceLabel = btn.dataset.deviceLabel || deviceId;
        this.openAlert('Blokir Device Ini?', 'Device "'+deviceLabel+'" tidak akan bisa membuka app ini lagi sampai kamu buka blokirnya dari sini. Lanjut?', [
          {text:'Batal', style:'cancel'},
          {text:'Blokir', style:'destructive', onClick:()=>this.handleBlockDevice(deviceId, deviceLabel)}
        ]);
      });
    });
  }
  ...
  /* =========================================================
     ...
     Device yang diblokir dicatat di koleksi Firestore terpisah
     "blocked_devices" (bukan di dalam activity_logs), dok ID-nya
     persis deviceId supaya initDeviceBlockCheck() bisa lookup
     langsung tanpa query. Blokir/buka-blokir cuma bisa dilakukan
     dari sesi Developer Mode yang sedang login (lihat Firestore
     Rules di FIREBASE_SETUP.md).
     ========================================================= */
  async handleBlockDevice(deviceId, deviceLabel){
    if(!window.firebaseAuth || !window.firebaseAuth.currentUser){
      this.openAlert('Sesi Login Habis', 'Login developer-mu sudah tidak aktif. Tutup panel ini dan login ulang, lalu coba blokir lagi.', [{text:'Oke', style:'cancel'}]);
      return;
    }
    try{
      await window.firebaseDb.collection('blocked_devices').doc(deviceId).set({
        deviceLabel: deviceLabel || deviceId,
        blockedAt: Date.now()
      });
      vibrate([12,30,12]);
      this.showToast('Device berhasil diblokir');
    }catch(err){
      console.error(err);
      this.openAlert('Gagal Memblokir', (err.message||'Periksa koneksi.') + '\n\nKalau pesannya soal "permission", Firestore Rules untuk koleksi "blocked_devices" belum ditambahkan — cek FIREBASE_SETUP.md.', [{text:'Oke', style:'cancel'}]);
    }
  }

  async handleUnblockDevice(deviceId){
    if(!window.firebaseAuth || !window.firebaseAuth.currentUser){
    ...
    try{
      await window.firebaseDb.collection('blocked_devices').doc(deviceId).delete();
      vibrate(10);
      this.showToast('Blokir dibuka');
      this.openBlockedDevicesManager();
    }catch(err){
      console.error(err);
      this.openAlert('Gagal Membuka Blokir', err.message||'Periksa koneksi.', [{text:'Oke', style:'cancel'}]);
    }
  }

  async openBlockedDevicesManager(){
    if(!window.firebaseDb){
      this.openAlert('Firebase Belum Siap', 'Lengkapi firebase-config.js terlebih dahulu.', [{text:'Oke', style:'cancel'}]);
      return;
    }
    this.openSheet('Device Diblokir', '<div class="empty-state"><p>Memuat…</p></div>');
    try{
      const snap = await window.firebaseDb.collection('blocked_devices').orderBy('blockedAt','desc').get();
      if(snap.empty){
        this.dom.sheetBody.innerHTML = '<div class="empty-state"><h4>Belum Ada yang Diblokir</h4><p>Device yang kamu blokir dari daftar log akan muncul di sini.</p></div>';
        return;
      }
      this.dom.sheetBody.innerHTML = snap.docs.map(doc=>{
        const d = doc.data();
        const date = new Date(d.blockedAt).toLocaleString('id-ID', {dateStyle:'medium', timeStyle:'short'});
        return '<div class="blocked-device-row">'
          + '<div class="blocked-device-info"><span class="blocked-device-label">'+(d.deviceLabel||doc.id)+'</span>'
          + '<span class="blocked-device-date">Diblokir '+date+'</span></div>'
          + '<button class="btn btn-secondary blocked-device-unblock" data-device-id="'+doc.id+'">Buka Blokir</button>'
          + '</div>';
      }).join('');
      this.dom.sheetBody.querySelectorAll('.blocked-device-unblock').forEach(btn=>{
        btn.addEventListener('click', ()=>this.handleUnblockDevice(btn.dataset.deviceId));
      });
    }catch(err){
      console.error(err);
      this.dom.sheetBody.innerHTML = '<div class="empty-state"><h4>Gagal Memuat</h4><p>'+(err.message||'Periksa koneksi & Firestore Rules.')+'</p></div>';
    }
  }
  ...
  handleCalculate(){
    ...
    setTimeout(()=>{
      try{
        ...
        console.error('Gagal menghitung:', err);
        this.showToast('Gagal menghitung: '+(err.message||'terjadi kesalahan tak terduga'));
      }finally{
        btn.innerHTML = originalHTML;
        btn.removeAttribute('disabled');
      }
